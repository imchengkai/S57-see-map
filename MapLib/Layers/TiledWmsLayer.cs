...
using EasyMap.Rendering.Exceptions;
using EasyMap.Utilities;
using EasyMap.Web.Wms;
using EasyMap.Web.Wms.Tiling;
...
namespace EasyMap.Layers
{
    ...
    public class TiledWmsLayer : Layer, ILayer
    {
        ...
        public TiledWmsLayer(string layername, string url, TimeSpan cachetime, WebProxy proxy)
        {
            ...
            if (HttpContext.Current != null && HttpContext.Current.Cache["EasyMap_WmsClient_" + url] != null)
            {
                _WmsClient = (Client) HttpContext.Current.Cache["EasyMap_WmsClient_" + url];
            }
            else
            {
                _WmsClient = new Client(url, _Proxy);
                if (HttpContext.Current != null)
                    HttpContext.Current.Cache.Insert("EasyMap_WmsClient_" + url, _WmsClient, null,
