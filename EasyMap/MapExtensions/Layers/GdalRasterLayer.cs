...
namespace EasyMap.Layers
{
    ...
    public class GdalRasterLayer : Layer, ICanQueryLayer, IDisposable
    {
        ...
        public GdalRasterLayer(BoundingBox box)
        {
            _envelope = box;
        }
        ...
        /// <param name="overviewlevel"></param>
        /// <param name="overviewzoom"></param>
        public bool SetOverView(Map map, out int overviewlevel, out int overviewzoom)
        {
            overviewlevel = -1;
            overviewzoom = 1;
            int viewcount=4;
            ...
            // bounds of section of image to be displayed
            double left = Math.Max(displayBbox.Left, _envelope.Left);
            double top = Math.Min(displayBbox.Top, _envelope.Top);
            double right = Math.Min(displayBbox.Right, _envelope.Right);
            double bottom = Math.Max(displayBbox.Bottom, _envelope.Bottom);
            ...
            for (int i = scale.Length - 1; i >= 0; i--)
            {
                ...
                Point bitmapBR = new Point((int)map.WorldToImage(trueImageBbox.BottomRight).X + 1,
                                     (int)map.WorldToImage(trueImageBbox.BottomRight).Y + 1);
                Point bitmapTL = new Point((int)map.WorldToImage(trueImageBbox.TopLeft).X,
...
#endif
        {
            ...
            if (dataset != null)
            {
                ...
                ////取得金字塔级别以及缩放比例
                //SetOverView(map, out overviewlevel, out overviewzoom);
                // bounds of section of image to be displayed
                ...
                {
                    _imagesize = new Size(dataset.GetRasterBand(1).GetOverview(overviewlevel).XSize, dataset.GetRasterBand(1).GetOverview(overviewlevel).YSize);
                    //_envelope = GetExtent();
                }
                ...
                bitmapTL = new Point((int)map.WorldToImage(trueImageBbox.TopLeft).X,
                                     (int)map.WorldToImage(trueImageBbox.TopLeft).Y);
                //GDAL_Data data = new GDAL_Data();
                //data.BitmapBR = bitmapBR;
                //data.BitmapTL = bitmapTL;
                //data.DisplayBbox = displayBbox;
                //data.DisplaySize = size;
                //data.FileList = new List<string>();
                //data.FileList.Add(Filename);
                //data.MapMaxY = map.Envelope.Max.Y;
                //data.MapMinX = map.Envelope.Min.X;
                //data.PixelHeight = map.PixelHeight;
                //data.PixelWidth = map.PixelWidth;



                //Bitmap mappp = new Bitmap(data.DisplaySize.Width, data.DisplaySize.Height);
                //Graphics ggg = Graphics.FromImage(mappp);
                //for (int i = 0; i < data.FileList.Count; i++)
                //{
                //    string file = data.FileList[i];
                //    Class1 layer = new Class1("a", file);
                //    layer.Render(ggg, data);
                //}
                //ggg.Dispose();
                //mappp.Save(@"d:\dd.bmp");
                ...
                //    Command.SendDrawPhotoData(data);
                //    while (!Command.MapMade)
                //    {
                //        Application.DoEvents();
                //    }
                //    if (g == null || Command.MadeMap == null || bitmapTL==null)
                //    {
                //        return;
                //    }
                //    g.DrawImage(Command.MadeMap, new Point(bitmapTL.X, bitmapTL.Y));
                //    return;
                //}
                ...
                try
                {
                    unsafe
                    {
                        ...
                        for (int i = 0; i < Bands; i++)
                        {
                            //displayImageLength = band[i].GetOverview(0).XSize;
                            //displayImageHeight = band[i].GetOverview(0).YSize;
                            ...
                            {
                                band[i].GetOverview(overviewlevel).ReadRaster(
                                (int)imageTL.X / overviewzoom,
                                (int)imageTL.Y / overviewzoom,
                                (int)(imageBR.X - imageTL.X / overviewzoom),
                                (int)(imageBR.Y - imageTL.Y / overviewzoom),
                                tempBuffer[i], displayImageLength, displayImageHeight, 0, 0);
                            }
                        ...
                        double dblXScale = (imageBR.X - imageTL.X / overviewzoom) / (displayImageLength / overviewzoom - 1);
                        double dblYScale = (imageBR.Y - imageTL.Y / overviewzoom) / (displayImageHeight / overviewzoom - 1);
...
#endif
                        int height = bitmapBR.Y - bitmapTL.Y;
                        int width = bitmapBR.X - bitmapTL.X;
                        for (PixY = 0; PixY < height; PixY++)
                        {
                            ...
                            for (PixX = 0; PixX < width; PixX++)
                            {
                                ...
                                int xindex = (int)((ImgX - imageLeft) / dblXScale);
                                int yindex = (int)((ImgY - imageTop) / dblYScale);
                                if (ImgX < imageTL.X || ImgY < imageTL.Y || xindex >= displayImageLength || yindex >= displayImageHeight || xindex<0||yindex<0)
                                    continue;

                                // color correction
                                for (int i = 0; i < Bands; i++)
                                {
                                    intVal[i] = buffer[i][xindex][yindex];
            ...
            foreach (Color color in FwToolsHelper.TransParentColors)
            {
                Bitmap mymap = new Bitmap(bitmap.Width, bitmap.Height);
                Graphics gg = Graphics.FromImage(mymap);
                gg.DrawImage(bitmap, 0, 0);
                gg.Dispose();
                mymap.MakeTransparent(color);
                bitmap = mymap;
            }
